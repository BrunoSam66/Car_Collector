...
var luz = new THREE.DirectionalLight("#ffffff", 0.12);
luz.lookAt(estrada);
...
var camaraOrtografica = new THREE.PerspectiveCamera(
    60,
    window.innerWidth / window.innerHeight,
    0.1,
    1000
);
camaraOrtografica.position.set(0, 0.8, 24);
...
function creatPlanoslaterais() {
    ...
    mundo.position.set(estrada.position.x, estrada.position.y + 5.1, estrada.position.z);
    return mundo;
}
...
function CreateBomb() {
    ...
    bomba.position.y = tampa.position.y - 0.22;
    bomba.position.z = tampa.position.z;
    tampa.scale.set(0.4, 0.2, 0.4);
    return bomb;
}
...
bombaLenta.position.set(2 * PositivoVsNeagativo(), -2.7, 3 * PositivoVsNeagativo());
CuboPosition(cubo1);
CuboPosition(cubo2);
CuboPosition(cubo3);
CuboPosition(cubo4);
...
function ModoNoturno() {
    if (!estadoAmbientLight && !estadoluzpointLight) { //sol e ambiente apagadas    
        Rightfarol.power = 3;
        Leftfarol.power = 3;
...
function Play(cubo) {
    if (DetectColision(cubo) == 0) {
        ...
        if (cubo.name == bombaLenta.name) { //coletou a bomba preteada      
            ...
            bombaLenta.position.set(2 * PositivoVsNeagativo(), -2.7, 3 * PositivoVsNeagativo()); //reposiciona a bombalenta
        } else {
            totalcubos++;
...
Bomba.position.set(limitex * PositivoVsNeagativo(), -2.8, (limiteZ - 15) * Math.random());

function goObject(obj) {
    if (!GameOver) {
        if (obj.position.z >= 15) {
            obj.position.set(limitex * PositivoVsNeagativo(), -2.8, (limiteZ - 1.3) * Math.random());
        } else if (DetectColision(Bomba) == 0) //colidiu com o carro
        {
            GameOver = true;
            obj.position.set(limitex * PositivoVsNeagativo(), -2.8, limiteZ * Math.random());
            tray_Again();
...
function NiveisVelocidade() {
    ...
    if (totalcubos >= 150 && totalcubos < 350) {
        ...
        //cubo1=loadFBX('../Objetos/camiao.fbx','../Imagens/moves.jpg',cubo1);
        //elemento distração, o cubo faz rotações fora da estrada a grande velocidade
        return 0.04;
    }
}
...
document.addEventListener("keydown", (ev) => {
    ...
    if (ev.keyCode == 79) { //ortográfica O  
        ...
        if (!camaraOrtograficaFlag) {
            ...
            cena.add(imag);
            xx.position.y = estrada.position.y + 4.3;
    ...
    if (ev.keyCode == 80) {
        //prespective P
        xx.position.y = estrada.position.y + 5.1;
    ...
    if (ev.keyCode == 50) {
        if (estadoluzDirectionalLight) {
            cena.remove(Rightfarol);
            cena.remove(Leftfarol);
    ...
    if (ev.keyCode == 51) {
        if (estadoAmbientLight) {
            cena.remove(luzAmbienteLeft);
            cena.remove(luzAmbienteRight);
...
function setScale(tamanho, tamanho1, tamanho2) {
...
function DeslocarCarroLeft() {
    ...
    if (!GameOver && !ganhou) {
        if (carro.position.x < -limitex) {
            carro.position.x = -limitex;
        } else carro.position.x -= velocidadeAndarx;
...
function SetCuboPosition(cubo) {
    ...
    if (cubo.name == CuboBonus.name) {
        CuboBonus.position.set(limitex * PositivoVsNeagativo(), -3, -25);
...
function highScore() {
    ...
    setInterval(function() {
        localStorage.clear();
    }, 1000 * 24 * 60 * 60); //limpa o histórico a cada 24horas
    //localStorage.clear(); 
    return saved;
}
